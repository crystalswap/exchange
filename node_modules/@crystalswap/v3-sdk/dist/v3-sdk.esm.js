import { MaxUint256, Token, Currency, WETH9, sqrt, Price, TokenAmount, Percent, ETHER, currencyEquals, TradeType, Fraction, CurrencyAmount, sortedInsert, validateAndParseAddress } from '@crystalswap/sdk-core';
import JSBI from 'jsbi';
import invariant from 'tiny-invariant';
import { defaultAbiCoder, Interface } from '@ethersproject/abi';
import { getCreate2Address } from '@ethersproject/address';
import { keccak256, pack } from '@ethersproject/solidity';
import { abi as abi$1 } from '@crystalswap/v3-periphery/artifacts/contracts/NonfungiblePositionManager.sol/NonfungiblePositionManager.json';
import { abi } from '@crystalswap/v3-periphery/artifacts/contracts/interfaces/ISelfPermit.sol/ISelfPermit.json';
import { abi as abi$2 } from '@crystalswap/v3-periphery/artifacts/contracts/SwapRouter.sol/SwapRouter.json';
...
function encodeRouteToPath(route, exactOutput) {
  var firstInputToken = route.input instanceof Token ? route.input : route.input === Currency.ETHER ? WETH9[route.chainId] : undefined;
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 4:
              ...
              return _context.abrupt("return", [new TokenAmount(outputToken, JSBI.multiply(outputAmount, NEGATIVE_ONE)), new Pool(this.token0, this.token1, this.fee, sqrtRatioX96, liquidity, tickCurrent, this.tickDataProvider)]);
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            ...
            case 4:
              ...
              return _context2.abrupt("return", [new TokenAmount(inputToken, inputAmount), new Pool(this.token0, this.token1, this.fee, sqrtRatioX96, liquidity, tickCurrent, this.tickDataProvider)]);
...
var Position = /*#__PURE__*/function () {
  ...
  _proto.minimumAmounts = function minimumAmounts(slippageTolerance) {
    ...
    var priceLower = this.pool.token0Price.raw.multiply(new Percent(1).subtract(slippageTolerance));
    ...
    get: function get() {
      if (this._token0Amount === null) {
        if (this.pool.tickCurrent < this.tickLower) {
          this._token0Amount = new TokenAmount(this.pool.token0, SqrtPriceMath.getAmount0Delta(TickMath.getSqrtRatioAtTick(this.tickLower), TickMath.getSqrtRatioAtTick(this.tickUpper), this.liquidity, false));
        } else if (this.pool.tickCurrent < this.tickUpper) {
          this._token0Amount = new TokenAmount(this.pool.token0, SqrtPriceMath.getAmount0Delta(this.pool.sqrtRatioX96, TickMath.getSqrtRatioAtTick(this.tickUpper), this.liquidity, false));
        } else {
    ...
    get: function get() {
      if (this._token1Amount === null) {
        if (this.pool.tickCurrent < this.tickLower) {
          ...
          this._token1Amount = new TokenAmount(this.pool.token1, SqrtPriceMath.getAmount1Delta(TickMath.getSqrtRatioAtTick(this.tickLower), this.pool.sqrtRatioX96, this.liquidity, false));
        } else {
          this._token1Amount = new TokenAmount(this.pool.token1, SqrtPriceMath.getAmount1Delta(TickMath.getSqrtRatioAtTick(this.tickLower), TickMath.getSqrtRatioAtTick(this.tickUpper), this.liquidity, false));
        }
      }
...
var Route = /*#__PURE__*/function () {
  function Route(pools, input, output) {
    ...
    var inputTokenIsInFirstPool = input instanceof Token && pools[0].involvesToken(input);
    var inputWethIsInFirstPool = input === ETHER && weth && pools[0].involvesToken(weth);
    var inputIsValid = inputTokenIsInFirstPool || inputWethIsInFirstPool;
    !inputIsValid ? process.env.NODE_ENV !== "production" ? invariant(false, 'INPUT') : invariant(false) : void 0;
    var noOutput = typeof output === 'undefined';
    var outputTokenIsInLastPool = output instanceof Token && pools[pools.length - 1].involvesToken(output);
    var outputWethIsInLastPool = output === ETHER && weth && pools[pools.length - 1].involvesToken(weth);
    var outputIsValid = noOutput || outputTokenIsInLastPool || outputWethIsInLastPool;
    !outputIsValid ? process.env.NODE_ENV !== "production" ? invariant(false, 'OUTPUT') : invariant(false) : void 0;
    /**
     * Normalizes token0-token1 order and selects the next token/fee step to add to the path
     * */

    var tokenPath = [input instanceof Token ? input : weth];
  ...
  _createClass(Route, [{
    ...
    get: function get() {
      ...
      !(this.input === Currency.ETHER) ? process.env.NODE_ENV !== "production" ? invariant(false, 'ETHER') : invariant(false) : void 0;
    ...
    get: function get() {
      ...
      !(this.output === Currency.ETHER) ? process.env.NODE_ENV !== "production" ? invariant(false, 'ETHER') : invariant(false) : void 0;
      return WETH9[this.chainId];
    }
    /**
...
var Trade = /*#__PURE__*/function () {
      ...
      return runtime_1.wrap(function _callee3$(_context3) {
        while (1) {
          switch (_context3.prev = _context3.next) {
            ...
            case 30:
              inputAmount = tradeType === TradeType.EXACT_INPUT ? amount : route.input === ETHER ? CurrencyAmount.ether(amounts[0].raw) : amounts[0];
              outputAmount = tradeType === TradeType.EXACT_OUTPUT ? amount : route.output === ETHER ? CurrencyAmount.ether(amounts[amounts.length - 1].raw) : amounts[amounts.length - 1];
        ...
        while (1) {
          switch (_context4.prev = _context4.next) {
            case 0:
              ...
              chainId = currencyAmountIn instanceof TokenAmount ? currencyAmountIn.token.chainId : currencyOut instanceof Token ? currencyOut.chainId : undefined;
            ...
            case 30:
              ...
              _context4.next = 35;
              return Trade.fromRoute(new Route([].concat(currentPools, [pool]), originalAmountIn.currency, currencyOut), originalAmountIn, TradeType.EXACT_INPUT);
        ...
        while (1) {
          switch (_context5.prev = _context5.next) {
            case 0:
              ...
              chainId = currencyAmountOut instanceof TokenAmount ? currencyAmountOut.token.chainId : currencyIn instanceof Token ? currencyIn.chainId : undefined;
              !(chainId !== undefined) ? process.env.NODE_ENV !== "production" ? invariant(false, 'CHAIN_ID') : invariant(false) : void 0;
            ...
            case 30:
              ...
              _context5.next = 35;
              return Trade.fromRoute(new Route([pool].concat(currentPools), currencyIn, originalAmountOut.currency), originalAmountOut, TradeType.EXACT_OUTPUT);
  ...
  _createClass(Trade, [{
    ...
    get: function get() {
      ...
      return (_this$_executionPrice = this._executionPrice) != null ? _this$_executionPrice : this._executionPrice = new Price(this.inputAmount.currency, this.outputAmount.currency, this.inputAmount.raw, this.outputAmount.raw);
    }
  }]);
...
var NonfungiblePositionManager = /*#__PURE__*/function (_SelfPermit) {
  ...
  NonfungiblePositionManager.encodeCollect = function encodeCollect(options) {
    ...
    var involvesETH = currencyEquals(options.expectedCurrencyOwed0.currency, ETHER) || currencyEquals(options.expectedCurrencyOwed1.currency, ETHER);
    ...
    if (involvesETH) {
      var ethAmount = currencyEquals(options.expectedCurrencyOwed0.currency, ETHER) ? options.expectedCurrencyOwed0.raw : options.expectedCurrencyOwed1.raw;
      var token = currencyEquals(options.expectedCurrencyOwed0.currency, ETHER) ? options.expectedCurrencyOwed1.currency : options.expectedCurrencyOwed0.currency;
      var tokenAmount = currencyEquals(options.expectedCurrencyOwed0.currency, ETHER) ? options.expectedCurrencyOwed1.raw : options.expectedCurrencyOwed0.raw;
  ...
  NonfungiblePositionManager.removeCallParameters = function removeCallParameters(position, options) {
    ...
    calldatas.push.apply(calldatas, NonfungiblePositionManager.encodeCollect(_extends({
      ...
      expectedCurrencyOwed0: expectedCurrencyOwed0.add(currencyEquals(expectedCurrencyOwed0.currency, ETHER) ? CurrencyAmount.ether(amount0Min) : new TokenAmount(expectedCurrencyOwed0.currency, amount0Min)),
      expectedCurrencyOwed1: expectedCurrencyOwed1.add(currencyEquals(expectedCurrencyOwed1.currency, ETHER) ? CurrencyAmount.ether(amount1Min) : new TokenAmount(expectedCurrencyOwed1.currency, amount1Min))
    }, rest)));
...
var SwapRouter = /*#__PURE__*/function (_SelfPermit) {
  ...
  SwapRouter.swapCallParameters = function swapCallParameters(trade, options) {
    ...
    if (options.inputTokenPermit) {
      !(trade.inputAmount.currency instanceof Token) ? process.env.NODE_ENV !== "production" ? invariant(false, 'NON_TOKEN_PERMIT') : invariant(false) : void 0;
    ...
    var value = currencyEquals(trade.inputAmount.currency, ETHER) ? amountIn : toHex(0); // flag for whether the trade is single hop or not

    var singleHop = trade.route.pools.length === 1; // flag for whether a refund needs to happen

    var mustRefund = currencyEquals(trade.inputAmount.currency, ETHER) && trade.tradeType === TradeType.EXACT_OUTPUT; // flags for whether funds should be send first to the router

    var outputIsETHER = currencyEquals(trade.outputAmount.currency, ETHER);
