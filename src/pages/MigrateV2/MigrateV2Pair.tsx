...
import { Fraction, Price, Token, TokenAmount, WETH9 } from '@crystalswap/sdk-core'
import { FACTORY_ADDRESS, JSBI } from '@crystalswap/v2-sdk'
...
import { FeeAmount, Pool, Position, priceToClosestTick, TickMath } from '@crystalswap/v3-sdk'
...
function V2PairMigration({
  ...
  const isNotCrystalswap = pairFactory.result?.[0] && pairFactory.result[0] !== FACTORY_ADDRESS
  ...
  }, [isNotCrystalswap, gatherPermitSignature, approveManually])
      ...
      .then((gasEstimate) => {
        return migrator
          ...
          .then((response: TransactionResponse) => {
            ReactGA.event({
              category: 'Migrate',
              action: `${isNotCrystalswap ? 'SushiSwap' : 'V2'}->V3`,
  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        This tool will safely migrate your {isNotCrystalswap ? 'SushiSwap' : 'V2'} liquidity to V3. The process is
        ...
        {chainId && migratorAddress && (
          <ExternalLink href={getEtherscanLink(chainId, migratorAddress, 'address')}>
            <TYPE.blue display="inline">Crystalswap migration contract↗</TYPE.blue>
          </ExternalLink>
        )}
        .
      </TYPE.body>
      ...
      <LightCard>
        <AutoColumn gap="lg">
          <RowBetween>
            ...
            <Badge variant={BadgeVariant.WARNING}>{isNotCrystalswap ? 'Sushi' : 'V2'}</Badge>
          </RowBetween>
          ...
          {noLiquidity && (
            <BlueCard style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <TYPE.body fontSize={14} style={{ marginBottom: 8, fontWeight: 500, opacity: 0.8 }} textAlign="center">
                You are the first liquidity provider for this Crystalswap V3 pool. Your liquidity will migrate at the
                current {isNotCrystalswap ? 'SushiSwap' : 'V2'} price.
              </TYPE.body>
                ...
                <AutoColumn gap="8px" style={{ marginTop: '12px' }}>
                  <RowBetween>
                    <TYPE.body fontWeight={500} fontSize={14}>
                      {isNotCrystalswap ? 'SushiSwap' : 'V2'} {invertPrice ? currency1.symbol : currency0.symbol} Price:{' '}
                      {invertPrice
          ...
          {largePriceDifference ? (
            <YellowCard>
              ...
              <TYPE.body fontSize={14} style={{ marginTop: 8, fontWeight: 400 }}>
                You should only deposit liquidity into Crystalswap V3 at a price you believe is correct. <br />
